...
class primalitytests extends \pages{
  function getContent(){
    return
    ...
    function check(list,offset){
      ...
      if (list == true){
        if (answer == true){
            document.primelist.javascriptlistoutput.value += Result + String.fromCharCode(13); // here so says done when present alert box
        }
        return answer;
      }else{
          document.primetest.javascriptoutput.value = Result; // here so says done when present alert box
      }
    }
    ...
    <table bgcolor="#e0faed" cellpadding="10" cellspacing="0" class="text" width="200" style="border:1px solid #444;"><tr><td>
